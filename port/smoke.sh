...
# is kept outside this repository with the scripts that need it. Point
# $CYTHERA_SYMBOLS at it to use it. The port runs perfectly well without it,
# printing addresses where it would print names, so it is passed only when it
# is actually present.
syms=()
for cand in "${CYTHERA_SYMBOLS:-}" "$root/reference/cythera_symbols.txt" \
            "$root/cythera_symbols.txt"; do
...
if [[ ! -f "$extract/Cythera.data" ]]; then
  ...
  mkdir -p "$extract"
  python3 "$root/port/binhex_decode.py" "$root/reference/game/Cythera.hqx" "$extract" >/dev/null
  python3 "$root/port/binhex_decode.py" "$root/reference/game/Cythera Data.hqx" "$extract" >/dev/null
fi
